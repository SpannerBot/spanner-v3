...
class DevEntitlementCog(commands.Cog):
    ...
    async def list_entitlements(
        ...
        entitlements = self.bot.entitlements(user=user, skus=sku_ids, guild=guild_id, exclude_ended=exclude_ended)
    ...
    @commands.is_owner()
    async def create_test_entitlement(self, ctx: commands.Context, sku_id: int, owner: int | discord.User):
        ...
        try:
            entitlement = await self.bot.http.create_test_entitlement(
                self.bot.user.id, dict(sku_id=sku_id, owner_id=owner.id, owner_type=owner_type)
            )
    ...
    @commands.is_owner()
    async def delete_test_entitlement(self, ctx: commands.Context, entitlement_id: int):
    ...
    async def list_skus(self, ctx: commands.Context):
        ...
        for sku in skus:
            paginator.add_line(
                "{0.id} - name={0.name!r} type={0.type!r} slug={0.slug!r} flags={1}".format(sku, dict(sku.flags)),
                empty=True,
            )
    ...
    async def free_trial(self, ctx: commands.Context):
        ...
        if already_entitled:
            return await ctx.reply("\N{CROSS MARK} This server is already entitled to premium features.", view=None)
        ...
        if (trial := await Premium.get_or_none(guild_id=ctx.guild.id)) is not None:
            if trial.is_expired:
                return await ctx.reply(
                    f"\N{CROSS MARK} Your free trial expired {discord.utils.format_dt(trial.end, 'R')}.", view=view
                )
            else:
                return await ctx.reply(
            ...
            try:
                trial = await Premium.create(
                    user_id=ctx.author.id, guild_id=ctx.guild.id, start=now, end=now + datetime.timedelta(days=30)
                )
    ...
    async def free_trial_info(self, ctx: commands.Context, guild_id: int = None):
        ...
        return await ctx.reply(
            ...
            "* Is expired: {1.is_expired!s}".format(
                ctx.guild, trial, discord.utils.format_dt(trial.start, "R"), discord.utils.format_dt(trial.end, "R")
            )
        )
    ...
    async def free_trial_grant(self, ctx: commands.Context, guild_id: int = None, days: int = 30):
        ...
        trial, _ = await Premium.update_or_create(
            guild_id=guild_id, defaults=dict(user_id=ctx.author.id, start=now, end=now + datetime.timedelta(days=days))
        )
        return await ctx.reply(
    ...
    async def free_trial_test(self, ctx: commands.Context, guild_id: int = None, allow_trial: bool = False):
        ...
        if not await entitled_to_premium(guild, allow_trial=allow_trial):
            ...
            if existing_trial:
                return await ctx.reply(
                    "\N{CROSS MARK} This server's free trial expired.", view=self.get_premium_view(ctx)
                )
            return await ctx.reply(
                "\N{CROSS MARK} This server is not entitled to premium features.", view=self.get_premium_view(ctx)
            )
        else:
            return await ctx.reply("\N{WHITE HEAVY CHECK MARK} This server is entitled to premium features.")
